...
select * from bank_details;
update  bank_details  set education='Graduation' where education='unknown';

SELECT min(BALANCE) FROM BANK_DETAILS;
SELECT SUM(BALANCE) FROM BANK_DETAILS WHERE LOAN='YES';
SELECT SUM(BALANCE) FROM BANK_DETAILS WHERE LOAN='NO';

SELECT SUM(BALANCE),LOAN FROM BANK_DETAILS GROUP BY LOAN ;

SELECT distinct(JOB) FROM BANK_DETAILS;

SELECT MIN(BALANCE),JOB FROM BANK_DETAILS GROUP BY JOB;

SELECT * FROM BANK_DETAILS WHERE AGE>=60;
SELECT * FROM BANK_DETAILS WHERE AGE=33 AND MARITAL='MARRIED';
SELECT * FROM BANK_DETAILS WHERE EDUCATION='PRIMARY' OR MARITAL='SINGLE' OR BALANCE<500;
...
select marital,sum(balance) from bank_details group by marital;
select *  from bank_details where age>60;
select * from bank_details where education="unknown";
select housing,count(housing) from bank_details group by housing;
select * from bank_details where month='october';
select * from bank_details;
select age,job,marital,balance from bank_details where balance=0;
select min(balance) from bank_details where;
 select * from bank_details where education='unknown' or marital='married' or balance>500;
